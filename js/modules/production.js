...
function buildTableRow(record) {
  ...
  return `
    <tr class="table-row">
      ...
      <td>${operatorName}</td>
      <td class="text-right td-value">${productionValue}</td>
 ...
 *   2. Read the operator rate from the form input.
 *   3. Stamp both into the payload as productPriceSnapshot / operatorRateSnapshot.
...
async function handleFormSubmit(e) {
  ...
  try {
    ...
    if (editingRecord) {
      ...
      showFeedback('Producción actualizada correctamente.', 'success');
...
async function handleDelete(recordId) {
  ...
  try {
    ...
    showFeedback('Registro de producción eliminado.', 'success');
