...
- On every new record, this module reads the product’s current sale price
- from productMap and stamps it as `productPriceSnapshot`.
- The operator rate entered in the form is stamped as `operatorRateSnapshot`.
- Both values are NEVER recalculated or overwritten after creation.
        ...
        <div class="form-group form-group--wide" id="prod-snapshot-panel" style="display:none;">
          <div class="snapshot-panel">
            ...
            <div class="snapshot-panel__values">
              <span>Precio producto al registrar:
                <strong id="prod-snapshot-price">—</strong>
              </span>
      ...
      <table class="data-table" id="production-table">
        <thead>
          <tr>
            ...
            <th>Turno</th>
            <th>Operario</th>
            <th class="text-right">Valor Producción</th>
...
// Valor de producción = quantity × productPriceSnapshot (immutable snapshot)
const productionValue = formatCurrency(
(record.quantity || 0) * (record.productPriceSnapshot || 0)
);
...
return `<tr class="table-row"> <td class="td-date">${escapeHTML(dateFormatted)}</td> <td class="td-machine">${machineName}</td> <td>${productName}</td> <td class="text-right td-quantity">${formatNumber(record.quantity)}</td> <td><span class="badge ${shiftClass}">${escapeHTML(record.shift || '—')}</span></td> <td>${operatorName}</td> <td class="text-right td-value">${productionValue}</td> <td class="text-center td-actions"> <button class="btn btn--ghost btn--xs" data-action="edit" data-id="${record.id}" title="Editar registro" >✎ Editar</button> <button class="btn btn--danger btn--xs" data-action="delete" data-id="${record.id}" title="Eliminar registro" >✕ Eliminar</button> </td> </tr>`;
}
...
- 1. Read the selected product’s current priceStandard from productMap.
- 1. Read the operator rate from the form input.
- 1. Stamp both into the payload as productPriceSnapshot / operatorRateSnapshot.
- These values are then frozen by the adapter’s update() method forever after.
- 
...
if (editingRecord) {
  ...
  // Resolve product once — used for snapshot, type validation, and inventory link
  const product = productMap.get(String(payload.productId));

  // Stamp product price snapshot from authoritative map (not a form field)
  payload.productPriceSnapshot = product ? (product.priceStandard || 0) : 0;
...
document.getElementById(‘prod-snapshot-price’).textContent =
formatCurrency(record.productPriceSnapshot);
...
// Update form chrome
document.getElementById(‘production-form-title’).innerHTML = `<span class="card__title-icon">✎</span> Editar Registro`;
...
// Restore form chrome
document.getElementById(‘production-form-title’).innerHTML = `<span class="card__title-icon">+</span> Nuevo Registro`;
document.getElementById(‘production-submit-btn’).innerHTML =
...
- 1. Date to     (inclusive)
- 1. Machine ID  (exact match)
- 1. Product ID  (exact match)
- 1. Operator ID (exact match)
- 
...
- productPriceSnapshot is resolved in handleFormSubmit (create only) by reading
- productMap — NOT from a form field — to ensure it reflects the authoritative
- stored price at the exact moment of creation.
- 
- @returns {Object}
  */
  ...
  // productPriceSnapshot: resolved in handleFormSubmit, not collected here
  };
  }
...
toast.innerHTML = `<span class="toast__icon" aria-hidden="true">${icons[type] ?? 'ℹ'}</span> <span class="toast__message">${escapeHTML(message)}</span> <span class="toast__close" aria-label="Cerrar">&times;</span>`;
