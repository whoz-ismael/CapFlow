...
async function handleFormSubmit(e) {
  ...
  try {
    if (editingProduct) {
      ...
      // Log the change — compute field-level diff
      const changes = buildDiff(editingProduct, payload, [
        'name', 'type', 'priceStandard', 'priceInvestor', 'active',
      ]);
      ChangeHistoryAPI.log({
        entity_type: 'product',
        entity_id:   String(editingProduct.id),
 ...
 *
 * @param {Object}   original  - The record before editing
 * @param {Object}   updated   - The new payload from the form
 * @param {string[]} fields    - Field names to compare
 * @returns {Object|null}      - { field: { before, after } } or null if nothing changed
 */
