...
// CLIENTS
//
// DB: id, name, rnc, phone, email, address, notes, active (bool),
//     created_at (timestamptz), updated_at (timestamptz)
// JS: id, name, rnc, phone, email, address, notes, active,
//     createdAt, updatedAt
// =============================================================================

function _clientFromDb(r) {
  return {
    id:        r.id,
    name:      r.name,
    rnc:       r.rnc,
...
export const ClientsAPI = {
  async getAll() {
    const { data, error } = await _sb.from('clients').select('*');
    if (error) throw new Error(error.message);
  ...
  async getById(id) {
    const { data, error } = await _sb.from('clients').select('*')
      .eq('id', String(id)).maybeSingle();
    if (error) throw new Error(error.message);
    return data ? _clientFromDb(data) : null;
  },
  ...
  async create(d) {
    ...
    const { data, error } = await _sb.from('clients').insert(row).select().single();
    if (error) throw new Error(error.message);
    return _clientFromDb(data);
  },

  async update(id, d) {
    const u = { updated_at: new Date().toISOString() };
    if (d.name    !== undefined) u.name    = (d.name || '').trim();
    if (d.rnc     !== undefined) u.rnc     = (d.rnc || '').trim();
    if (d.phone   !== undefined) u.phone   = (d.phone || '').trim();
    if (d.email   !== undefined) u.email   = (d.email || '').trim();
    if (d.address !== undefined) u.address = (d.address || '').trim();
    if (d.notes   !== undefined) u.notes   = (d.notes || '').trim();
    ...
    const { data, error } = await _sb.from('clients').update(u)
      .eq('id', String(id)).select().single();
    if (error) throw new Error(error.message);
    return _clientFromDb(data);
  },
  ...
  async setStatus(id, active)  { return this.update(id, { active }); },
};
